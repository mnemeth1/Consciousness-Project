# Public snapshot scope and provenance
...
`PUBLICATION_MANIFEST.json` records each exported file's SHA-256, its source path and source SHA-256 where copied, whether it remains byte-identical, and the applied transformation policy. The manifest does not hash itself. Original-source hashes mentioned inside historical reports concern the withheld research archive; they are not a claim that those originals are in this repository. Their preservation here does not make the public copy a complete audit archive.

Many historical reports refer to task-relative notes, source captures and audit files that are intentionally omitted. Such references remain provenance identifiers, not promises of downloadable content. The source and locator appendix and source ledger provide bibliographic routes. Omitted operational audit records cannot be independently reconstructed from this public snapshot alone. Exact source acquisition and independent reading are still required for replication.
